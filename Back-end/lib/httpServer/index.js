...
class HttpServer {
    constructor(node, blockchain, operator, miner) {
        ...
        const sslOptions = {
            key: fs.readFileSync('./ssl/key.pem'), // Replace with the actual path if needed
        ...
        this.app.use(cors({
            // origin: 'https://192.168.33.253:3000', // Allow requests from the Next.js frontend
            origin: 'https://localhost:3000', // Allow requests from the Next.js frontend
        ...
        this.app.post('/operator/wallets', (req, res) => {
            ...
            if (userID.length <= 4) throw new HTTPError(400, 'userID must contain more than 4 words');

            // check if userID is duplicated
            let walletFound = operator.getWalletByUserId(userID);
            if (walletFound) throw new HTTPError(500, 'userID must not be duplicated');

            // let newWallet = operator.createWalletFromPassword(password);
            let newWallet = operator.createWalletFromPasswordAndUserID(password, userID);
        ...
        this.app.post('/operator/wallets/auth', (req, res) => {
            let password = req.body.password;
            let userID = req.body.userID;
            ...
            if (!operator.checkWalletPassword(walletFound.id, passwordHash)) throw new HTTPError(403, `Invalid password for wallet '${walletId}'`);

            console.log(passwordHash + " and " + password);
        ...
        this.app.post('/operator/wallets/:walletId/transactions', (req, res) => {
            ...
            try {
                ...
                let newTransaction = operator.createTransaction(walletId, req.body.fromAddress, req.body.toAddress, req.body.amount, req.body['changeAddress'] || req.body.fromAddress, req.body.stuId, req.body.teacherId, req.body.eventId, req.body.deadline, req.body.remark);
